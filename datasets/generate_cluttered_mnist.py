...
generate_cluttered_mnist.py - Generate a cluttered 
"""
...
def create_sample(digits, out_shape, idx):
    """ Generate an {out_shape} sized sample of {digits} """

    # Dimensions of input
    input_x = digits[0].shape[0]
    input_y = digits[0].shape[1]
...
def generate_dataset(n_samples, data, labels, in_shape, out_shape, digit_count=3):
    """ Create the Cluttered MNIST Data set with the specified parameters """

    # Declare data and label arrays
    dataset = np.zeros((n_samples, out_shape[0], out_shape[1]))
    labelset = np.zeros((n_samples, digit_count))
    gt = np.zeros((n_samples, digit_count, in_shape[0], in_shape[1]))
    blurred = np.zeros((n_samples, out_shape[0], out_shape[1]))

    # Generate distortion data
    distortion_count = 100000
    num_distortions = 6
    distortions = generate_distortions(data, distortion_count, num_samples)
    ...
    for i in tqdm(range(num_samples)):
        ...
        # Append the digits for ground truth comparison later
        gt[i] = np.array(_digits)

        # Create a canvas sample combining the digits
        output = create_sample(_digits, out_shape, i)

        # Labels array
        labelset[i] = np.array(_labels)

        # Add distortions to canvas
        dataset[i] = add_distortions(output, num_distortions, distortions)

        # Create Blurred "Salience"
        blurred[i] = gaussian_filter(output, sigma=5)

    print(labelset.shape)

    return (dataset.astype('uint32'), labelset.astype('int32'),
            gt.astype('uint8'), blurred.astype('uint8'))
...
if __name__ == "__main__":
    ...
    samples_arr = np.vstack(samples).reshape(num_samples, 100, 100)
    out = plot_n_by_n_images(samples_arr, epoch="0", folder="ClutteredMNIST",
                             n=10, shape=out_shape)

    # Generate training data set and save to pickle file
    td, tl, gt, b = generate_dataset(num_samples, mnist_train_data,
                                     mnist_train_labels, in_shape, out_shape)
    ...
    plt.figure()
    plt.imshow(ground_truth[idx][0])
    plt.show()

    # Dump to pickle file
    filename = "./ClutteredMNIST/train.pickle"
    outfile = open(filename, "wb")
    pickle.dump([[train_data, ground_truth], train_labels, blurred], outfile)
    outfile.close()

    # Generate testing data set and save to pickle file
    num_samples = num_samples // 10
    all_digits = mnist_test_data
    td, tl, gt, b = generate_dataset(num_samples, mnist_test_data,
                                     mnist_test_labels, in_shape, out_shape)
    ...
    # Dump to pickle file
    filename = "./ClutteredMNIST/test.pickle"
    outfile = open(filename, "wb")
    pickle.dump([[test_data, ground_truth], test_labels], outfile)
    outfile.close()
